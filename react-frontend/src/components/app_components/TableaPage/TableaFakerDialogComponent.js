...
import tableaFakerFactory from "./tableaFakerFactory";
...
const TableaFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = tableaFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("tablea").create(elem));
...
export default connect(mapState, mapDispatch)(TableaFakerDialogComponent);
